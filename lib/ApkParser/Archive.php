	...
	*/
	public function getFromName($name,$length = NULL,$flags = NULL) {
		if(strtolower(substr($name,-4)) == '.xml') {
			$xmlParser = new \ApkParser\XmlParser(new \ApkParser\Stream($this->getStream($name)));
	...
	/**
	* Returns an \ApkParser\Stream whick contains AndroidManifest.xml
	* @return ApkStream
	*/
	public function getManifestStream() {
		return new \ApkParser\Stream($this->getStream('AndroidManifest.xml'));
	}
	...
	public function extractTo($destination,$entries = NULL) {
		if($extResult = parent::extractTo($destination,$entries)) {
			...
			foreach($xmlFiles as $xmlFile) {
				// TODO : Remove this if check , if ApkXml can parse! amk!
